...
/*
 * Variante 3 da Tarefa A: laço irregular com schedule(guided, chunk).
...
int main(int argc, char **argv) {
	...
	if (chunk <= 0) {
		fprintf(stderr, "Erro: chunk deve ser positivo para schedule(guided). chunk=%lld\n", chunk);
		return 1;
	}

	/* Para esta variante, esperamos schedule == "guided". */
	if (strcmp(schedule_str, "guided") != 0) {
		fprintf(stderr,
			"Aviso: variante 3 usa sempre schedule(guided,chunk); argumento schedule='%s' será ignorado.\n",
			schedule_str);
	}
